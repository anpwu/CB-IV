...
def mmd2_rbf(X,t,p,sig,t_mean=0.5):
...
def wasserstein(X,t,p,lam=10,its=10,sq=False,backpropT=False,t_mean=0.5):
    ...
    a = tf.concat([p*tf.ones(tf.shape(tf.where(t>=t_mean)[:,0:1]))/nt, (1-p)*tf.ones((1,1))],0)
    b = tf.concat([(1-p)*tf.ones(tf.shape(tf.where(t<t_mean)[:,0:1]))/nc, p*tf.ones((1,1))],0)
