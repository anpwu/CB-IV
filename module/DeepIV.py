...
def run(exp, args, dataDir, resultDir, train, val, test, device):
    ...
    dropout_rate = min(1000./(1000. + args.num), 0.5)
    epochs = min(int(1000000./float(args.num)),100)
    if args.num_reps > 80:
        epochs = 30
    
    log(logfile, f"Run {exp}/{args.num_reps}")
    log(_logfile, f"Run {exp}/{args.num_reps}", False)

    mse_dict={"valid": [], "test": [], "train": []}

    # Build and fit treatment model

    instruments = Input(shape=(train_v.shape[1],), name="instruments")
    features = Input(shape=(train_x.shape[1],), name="features")
    treatment_input = Concatenate(axis=1)([instruments, features])

    if args.deepiv_t_loss == 'binary_crossentropy':
        est_treat = feed_forward_net(treatment_input, lambda x: binary_crossentropy_output(x),
                                               hidden_layers=args.deepiv_hidden,
                                               dropout_rate=dropout_rate, l2=0.0001,
                                               activations=args.deepiv_act)
    else:
        est_treat = feed_forward_net(treatment_input, lambda x: mixture_of_gaussian_output(x, args.deepiv_n_components),
                                               hidden_layers=args.deepiv_hidden,
                                               dropout_rate=dropout_rate, l2=0.0001,
                                               activations=args.deepiv_act)

    treatment_model = Treatment(inputs=[instruments, features], outputs=est_treat)
    treatment_model.compile('adam', loss=args.deepiv_t_loss, n_components=args.deepiv_n_components)

    treatment_model.fit([train_v, train_x], train.t, epochs=epochs, batch_size=args.deepiv_batch_size)

    # Build and fit response model

    treatment = Input(shape=(train.t.shape[1],), name="treatment")
    ...
    response_model.compile('adam', loss=args.deepiv_y_loss)
    response_model.fit([train_v, train_x], train.y, epochs=epochs, verbose=1,
                       batch_size=args.deepiv_batch_size, samples_per_batch=2)

    mu0_test = response_model.predict([test_x, test.t-test.t]).mean()
    mu1_test = response_model.predict([test_x, test.t-test.t+1]).mean()
    pred_ate_test = mu1_test-mu0_test

    mu0_train = response_model.predict([train_x, train.t-train.t]).mean()
    mu1_train = response_model.predict([train_x, train.t-train.t+1]).mean()
    pred_ate_train = mu1_train-mu0_train
    
    return pred_ate_train, pred_ate_test